...
def page_exists(title): #ritorna -1 se la pagina non esiste, altrimenti ritorna il page_id della pagina.
...
def epoca_naz_wiki(bio_nome):
	...
	if ricerca_luogo is None:
		...
		luogo = ricerca_sesso.group(1)
	
	return {'Epoca':epoca, 'Nazionalità':nazionalità, 'Nascita':nascita,'Morte':morte, 'Sesso':sesso,'Luogo_Nascita':luogo}
	


lista_autori = open("/home/masterbd/progetto/autori_a_z.txt","r").readlines() #questo file è quello che viene generato dallo scraping di liberliber
lista_aut_wiki = open("/home/masterbd/progetto/autori+wiki.txt","r") #deve esistere un file con questo nome in cui andrà la lista degli autori presenti su wikipedia
lista_mancanti = open("/home/masterbd/progetto/autori_mancanti.txt","r") #deve esistere un file con questo nome in cui andranno gli autori NON presenti su wikipedia
...
if len(lis_wiki)==0:
	lista_aut_wiki = open("/home/masterbd/progetto/autori+wiki.txt","w")
	lista_aut_wiki.write("Nome e Cognome | Cognome, Nome (originale) | Wiki Page_id | Epoca | Nazionalità\n")
	lista_aut_wiki.close()
lista_aut_wiki = open("/home/masterbd/progetto/autori+wiki.txt","a")
lista_mancanti = open("/home/masterbd/progetto/autori_mancanti.txt","a")
